...
/**
 * Formation geometry for planner tools. Read-only, never mutates the graph.
 * Local frame: +dLat forward (toward facing point), +dLng right.
 */
@Service
public class FormationService {

    /** ~200 m at mid-latitudes. */
    public static final double DEFAULT_SPACING_METERS = 200.0;
    /** Default form-up standoff short of the AOI (~2 km). */
    public static final double DEFAULT_STANDOFF_METERS = 2000.0;
    /** Rough meters-per-degree at mid-latitudes. Good enough for demos. */
    ...
    /**
     * Point on leader -> AOI line, standoffMeters short of the AOI.
     * Leader on top of AOI? Form up due south instead.
     */
    ...
    /** Preview with default north-facing orientation. */
    ...
    /** Assign droneIds to slots around center. Optional facing point rotates WEDGE/LINE toward it. */
    public FormationPreview preview(
        FormationType type,
        double centerLat,
        double centerLng,
    ...
    /** Rotate offsets so +forward points at the facing point. RING looks the same either way. */
    ...
    /** (dLat, dLng) offsets: +dLat forward, +dLng right. */
    ...
    /** Line along the right axis; broadside toward target after rotation. */
    ...
    /** V with apex at center. After rotation, apex leads toward the facing point. */
